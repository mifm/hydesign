...
class wpp_cost_pp:
    ...
    def compute(self, Nwt, hh, d, p_rated, wind_t):
        """ Computing the CAPEX and OPEX of the wind power plant.
        ...
        CAPEX_w = scale*(
            wind_turbine_cost + \
            wind_civil_works_cost) * (Nwt * p_rated)
        OPEX_w = wind_fixed_onm_cost * (Nwt * p_rated) + \
                            mean_aep_wind * wind_variable_onm_cost * p_rated/p_rated_ref
...
class pvp_cost_pp:
    ...
    def compute(self, solar_MW, DC_AC_ratio):
        """ Computing the CAPEX and OPEX of the solar power plant.
        ...
        solar_inverter_cost= self.solar_inverter_cost
        solar_fixed_onm_cost = self.solar_fixed_onm_cost
        CAPEX_s = (solar_PV_cost + solar_hardware_installation_cost ) * solar_MW * DC_AC_ratio + \
                              solar_inverter_cost * solar_MW
    ...
    def compute_partials(self, solar_MW, DC_AC_ratio):
        ...
        d_CAPEX_s_d_solar_MW = (solar_PV_cost + solar_hardware_installation_cost ) * DC_AC_ratio + \
             solar_inverter_cost * DC_AC_ratio_tech_ref/DC_AC_ratio 
        d_CAPEX_s_d_DC_AC_ratio = (solar_PV_cost + \
            solar_hardware_installation_cost + solar_inverter_cost * DC_AC_ratio_tech_ref/(- DC_AC_ratio ** 2)) * solar_MW
...
class shared_cost_pp:
    ...
    def compute(self, G_MW, Awpp, Apvp):
        """ Computing the CAPEX and OPEX of the shared land and infrastructure.
...
class ptg_cost_pp:
    ...
    def compute(self, ptg_MW, HSS_kg, m_H2_offtake_t):
        ...
        CAPEX_ptg = ptg_MW * (electrolyzer_capex_cost + electrolyzer_power_electronics_cost) + storage_capex_cost * HSS_kg + \
                               (m_H2_offtake_t.mean()*self.yearly_intervals * transportation_cost * transportation_distance) 
