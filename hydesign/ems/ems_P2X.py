...
class ems_P2X_pp:
    ...
    def compute(self, wind_t, solar_t, price_t, b_P, b_E, G_MW, battery_depth_of_discharge, battery_charge_efficiency,
                peak_hr_quantile, cost_of_battery_P_fluct_in_peak_price_ratio, n_full_power_hours_expected_per_day_at_peak_price, price_H2,
                ptg_MW, storage_eff, ptg_deg, hhv, m_H2_demand_t, HSS_kg, penalty_factor_H2, min_power_standby):
        ...
        P_HPP_ts, P_curtailment_ts, P_charge_discharge_ts, P_ptg_ts, P_ptg_SB_ts, E_SOC_ts, m_H2_ts, m_H2_offtake_ts, LoS_H2_ts, penalty_ts = ems_cplex_P2X(
            ...
            P_batt_MW = float(b_P),
            E_batt_MWh_t = WSPr_df.E_batt_MWh_t,
            hpp_grid_connection = float(G_MW),
            battery_depth_of_discharge = float(battery_depth_of_discharge),
            charge_efficiency = float(battery_charge_efficiency),
            price_H2 = float(price_H2),
            ptg_MW = float(ptg_MW),
            HSS_kg = float(HSS_kg),
            storage_eff = float(storage_eff),
            ptg_deg = float(ptg_deg),
            hhv = float(hhv),
            m_H2_demand_ts = WSPr_df.m_H2_demand_t,
            H2_storage_t = WSPr_df.H2_storage_t,
            penalty_factor_H2 =float(penalty_factor_H2),
            eff_curve=self.eff_curve,
            min_power_standby = float(min_power_standby),
            load_min_penalty_factor = float(self.load_min_penalty_factor),
            peak_hr_quantile = float(peak_hr_quantile),
            cost_of_battery_P_fluct_in_peak_price_ratio = float(cost_of_battery_P_fluct_in_peak_price_ratio),
            n_full_power_hours_expected_per_day_at_peak_price = float(n_full_power_hours_expected_per_day_at_peak_price),
            electrolyzer_eff_curve_type = self.electrolyzer_eff_curve_type
        )

        # Extend (by repeating them and stacking) all variable to full lifetime 
        price_t_ext = expand_to_lifetime(
            price_t, life_y=self.life_y, intervals_per_hour=self.intervals_per_hour, weeks_per_season_per_year = self.weeks_per_season_per_year)
...
def ems_longterm_P2X_proportional(
    wind_ts,
    solar_ts,
    price_ts,
    P_batt_MW,
    ...
    """Long term EMS solver implemented in cplex

    Parameters
    ----------
    
        Same as ems_cplex_P2X

    Returns
    -------
        Same as ems_cplex_P2X
        but with a proportional approach to the long term optimization
        and no penalty for not reaching expected energy production at peak hours
        or rapid fluctuations of the battery power.
    
    """
    a = (P_wind_deg + P_solar_deg) / (P_wind + P_solar)
