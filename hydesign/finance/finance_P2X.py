...
class finance_P2X_pp:
    ...
    def compute(self, hpp_t, m_H2_t, m_H2_offtake_t, m_H2_demand_t_ext, P_ptg_t,
                hpp_curt_t, price_H2, penalty_factor_H2, price_t_ext, penalty_t, 
                CAPEX_w, CAPEX_s, CAPEX_b, CAPEX_el, CAPEX_ptg,
                OPEX_w, OPEX_s, OPEX_b, OPEX_el, OPEX_ptg,
        ...
        df['hpp_t'] = hpp_t
        df['m_H2_t'] = m_H2_t
        df['m_H2_offtake_t'] = m_H2_offtake_t
        df['m_H2_demand_t_ext'] = m_H2_demand_t_ext
        df['P_ptg_t'] = P_ptg_t
        df['hpp_curt_t'] = hpp_curt_t
        price_H2 = np.atleast_1d(price_H2)
        penalty_factor_H2 = np.atleast_1d(penalty_factor_H2)
        price_t = price_t_ext
        ...
        # Compute yearly revenues and cashflow
        revenues = calculate_revenues_P2X(price_H2, price_t, penalty_factor_H2, df)
        CAPEX = CAPEX_w + CAPEX_s + \
            CAPEX_b + CAPEX_el + CAPEX_ptg
        OPEX = OPEX_w + OPEX_s + \
            OPEX_b + OPEX_el + OPEX_ptg + water_consumption_cost
        
        CAPEX_LCOE = CAPEX_w + CAPEX_s + \
            CAPEX_b + CAPEX_el 
        OPEX_LCOE = OPEX_w + OPEX_s + \
            OPEX_b + OPEX_el 
        ...
        # LCOH calculation using LCOE
        OPEX_ptg = OPEX_ptg + water_consumption_cost
        CAPEX_ptg = CAPEX_ptg
        hpp_discount_factor_H2 = ptg_WACC
        OPEX_ptg_el = LCOE*np.sum(P_ptg_t) #operational cost for the electrilcity consumed to produce hydrogen
        level_costs_H2 = np.sum(OPEX_ptg / (1 + hpp_discount_factor_H2)**iy) + OPEX_ptg_el + CAPEX_ptg
        AHP_per_year = df.groupby('i_year').m_H2_t.mean()*365*24
        level_AHP = np.sum(AHP_per_year / (1 + hpp_discount_factor_H2)**iy)
        ...
        mean_AHP_per_year = np.mean(AHP_per_year)
        if level_AHP > 0:
            LCOH = level_costs_H2 / (level_AHP) # in Euro/kg
        else:
            LCOH = 1e6
        ...
        Revenue = np.sum(revenues.values.flatten())
        revenues_mean = revenues.values.flatten().mean()
        # outputs['annual_P_ptg'] = mean_P_ptg_per_year
        # outputs['mean_AEP'] = mean_AEP_per_year
        # outputs['mean_Power2Grid'] = mean_Power2Grid_per_year
        # outputs['annual_H2'] = mean_AHP_per_year
        ...
        return (CAPEX, OPEX, NPV, IRR, NPV_over_CAPEX, LCOE, LCOH, Revenue, 
                mean_P_ptg_per_year, mean_AEP_per_year, mean_AHP_per_year,
