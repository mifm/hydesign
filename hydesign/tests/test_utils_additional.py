...
def test_hybridization_shifted_and_sample_mean():
    ...
    comp = hybridization_shifted(N_limit=N_limit, life_y=life_y, N_time=0, life_h=life_h)
    prob = om.Problem()
...
def test_component_wrapper_gradients():
    ...
    comp = ComponentWrapper(inputs=[('x', {'val':0.0}), ('y', {'val':0.0})],
                            outputs=[('f', {'val':0.0})],
                            function=func,
...
def test_generate_availability_ensamble_small():
    ds = generate_availability_ensamble(ts_start='2030-01-01 00:00',
