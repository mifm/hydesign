...
class hpp_model(hpp_base):
    ...
    def __init__(
        ...
        **kwargs
        ...
        n_full_power_hours_expected_per_day_at_peak_price = sim_pars['n_full_power_hours_expected_per_day_at_peak_price']
        ...
        Solar_cost = pvp_cost_pp(solar_PV_cost=sim_pars['solar_PV_cost'],
        ...
        Batt_cost = battery_cost_pp(battery_energy_cost=sim_pars['battery_energy_cost'],
        ...
        Shared_cost = shared_cost_pp(hpp_BOS_soft_cost=sim_pars['hpp_BOS_soft_cost'],
                        hpp_grid_connection_cost=sim_pars['hpp_grid_connection_cost'],
                        land_cost=sim_pars['land_cost'],)
        
        Finance = finance_pp(N_time = N_time, 
        ...
        def pywake_ref(ws, wd, **kwargs):
            ...
            loads_ref_ext = expand_to_lifetime(sim_res.loads(normalize_probabilities=False, method='OneWT')['DEL'].values, life_y=life_y, intervals_per_hour=intervals_per_hour, axis=2)
        ...
        def pywake(ws, wd, yaw, loads_ref_ext, **kwargs):
            ...
            loads = sim_res.loads(normalize_probabilities=False, method='OneWT')['DEL'].values
        ...
        def pv(surface_tilt, surface_azimuth, solar_MW, DC_AC_ratio, **kwargs):
            solar_t, Apvp = PVP.compute(surface_tilt, surface_azimuth, solar_MW, land_use_per_solar_MW, DC_AC_ratio)
            solar_t_ext = expand_to_lifetime(solar_t, life_y=life_y, intervals_per_hour=intervals_per_hour)
        ...
        def ems(wind_t, solar_t, b_P, b_E, 
            ...
            (price_t_ext, #wind_t_ext, solar_t_ext, 
             ...
             penalty_t) = EMS.compute(wind_t, solar_t, price_t, b_P, b_E, G_MW,
        ...
        def ems_long_term(SoH_all, solar_t_ext_deg, wind_t_ext, solar_t_ext, price_t_ext, b_E,
        ...
        def finance(hpp_t_with_deg, penalty_t_with_deg, price_t_ext, CAPEX_w, CAPEX_s, CAPEX_b, CAPEX_sh, OPEX_w,
                    OPEX_s, OPEX_b, OPEX_sh, **kwargs):
            (CAPEX, OPEX, revenues_mean, NPV, IRR, NPV_over_CAPEX, LCOE, mean_AEP, penalty_lifetime,
             break_even_PPA_price) = Finance.compute(hpp_t_with_deg, penalty_t_with_deg, price_t_ext, CAPEX_w, CAPEX_s,
                                                   CAPEX_b, CAPEX_sh, OPEX_w, OPEX_s, OPEX_b, OPEX_sh, wind_WACC,
                                                   solar_WACC, battery_WACC, tax_rate)
            return [CAPEX, OPEX, revenues_mean, NPV, IRR, NPV_over_CAPEX, LCOE, mean_AEP, penalty_lifetime,
                    break_even_PPA_price]


        comps = dict(
        abl_comp = ComponentWrapper([('hh',), ],
                                    [('ws', {'shape': [N_time]}),
                                     ('wd', {'shape': [N_time]}),
                                      ],
                                    abl,
        ...
        pywake_ref_comp = ComponentWrapper([('ws', {'shape': [N_time]}),
                                            ('wd', {'shape': [N_time]}),
                                            ],
        ...
        pywake_comp = ComponentWrapper([('ws', {'shape': [N_time]}),
                                        ('wd', {'shape': [N_time]}),
                                        ('yaw', {'shape': [N_time]}),
                                        ('loads_ref_ext', {'shape': [n_loads, n_wt, life_intervals]}),
                                        ],
        ...
        pv_comp = ComponentWrapper([('surface_tilt',),
                                    ('surface_azimuth',),
                                    ('solar_MW',),
                                    ('DC_AC_ratio',),
                                    ],
        ...
        ems_comp = ComponentWrapper([('wind_t', {'shape': [N_time]}),
                                    ('solar_t', {'shape': [N_time]}),
                                    ('b_P',),
                                    ('b_E',),
                                    ('cost_of_battery_P_fluct_in_peak_price_ratio',),
        ...
        battery_loss_comp = ComponentWrapper([('SoH', {'shape': [life_intervals]})],
                                             [('SoH_all', {'shape': [life_intervals]}),
                                              ],
                                             battery_loss,
                                             partial_options=[{'dependent': False, 'val': 0}],),

        ems_long_term_comp = ComponentWrapper([('SoH_all', {'shape': [life_intervals]}),
        ...
        wind_cost_comp = ComponentWrapper([('Nwt',), 
                                           ('hh',), 
                                           ('d',), 
                                           ('p_rated',), 
                                           ('wind_t', {'shape': [N_time]}), 
        ...
        solar_cost_comp = ComponentWrapper([('solar_MW',), 
                                           ('DC_AC_ratio',), 
                                           ],
                                          [('CAPEX_s', ),
                                           ('OPEX_s', ),
        ...
        batt_cost_comp = ComponentWrapper([('b_E',), 
                                           ('b_P',), 
                                           ('SoH', {'shape': [life_intervals]}), 
                                           ],
                                          [('CAPEX_b', ),
        ...
        shared_cost_comp = ComponentWrapper([('Awpp',), 
                                             ('Apvp',), 
                                             ],
                                            [('CAPEX_sh', ),
                                             ('OPEX_sh', ),
        ...
        finance_comp = ComponentWrapper([('hpp_t_with_deg', {'shape': [life_intervals]}),
                                          ('penalty_t_with_deg', {'shape': [life_intervals]}),
                                          ('price_t_ext', {'shape': [life_intervals]}),
                                          ('CAPEX_w', ),
                                          ('CAPEX_s', ),
...
if __name__ == '__main__':
    ...
    start = time.time()
            ...
            # Wind plant design
    x=dict(clearance=55.0, sp=257.0, p_rated=10.0, Nwt=n_wt, wind_MW_per_km2=5.917,
           # PV plant design
           solar_MW=75,  surface_tilt=28.125, surface_azimuth=191.250, DC_AC_ratio=1.479,
           # Energy storage & EMS price constrains
           b_P=27, b_E_h=4, cost_of_battery_P_fluct_in_peak_price_ratio=8.750,
           # Wind turbine control
           yaw=yaw)
